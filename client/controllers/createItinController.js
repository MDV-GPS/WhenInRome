...
function createItinController($scope, $location, $http, $window, UserFactory, HttpFactory, ProfileFactory, ParamsFactory) {
  ...
  // GETLOCATION ALLOWS US TO GET THE CURRENT POSITION OF
  // THE USER AS WELL AS CREATE THE authorZip FROM GEOLOCATION
  ...
  $scope.openProfile = () => {
    if ($scope.menuStyle === '') $scope.menuStyle = 'openMenu';
    else $scope.menuStyle = '';
  };
}
